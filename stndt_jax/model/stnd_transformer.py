...
import optax
from stndt_jax.loss_function.losses import poisson_nll_loss
...
class STNDT(eqx.Module):
    ...
    # Masks (static)
    src_mask: Optional[jnp.ndarray] = eqx.field(static=True)
    spatial_src_mask: Optional[jnp.ndarray] = eqx.field(static=True)
    config: Dict[str, Any] = eqx.field(static=True)
    ...
    def __init__(self, config, trial_length, num_neurons, max_spikes, key=None):
        ...
        if config.get("LOSS").get("TYPE") == "poisson":
            ...
            if not config.get("LOGRATE", True):
                self.src_decoder = eqx.nn.Sequential(
                    [self.src_decoder, eqx.nn.Lambda(jax.nn.relu)]
                )
        elif config.get("LOSS").get("TYPE") == "cel":
            self.src_decoder = eqx.nn.Sequential(
                eqx.nn.Linear(
                    self.get_factor_size(),
                    config.get("MAX_SPIKE_COUNT") * self.num_neurons,
                    key=src_decoder_layers_keys[0],
                )
            )  # log-likelihood
    ...
    def info_nce_loss(self, features):
        ...
        # Remove diagonal elements (self-similarity) using JAX-compatible operations
        # Create mask for off-diagonal elements
        n = labels.shape[0]
        i_indices = jnp.repeat(jnp.arange(n), n-1)
        j_indices = jnp.concatenate([
            jnp.concatenate([jnp.arange(i), jnp.arange(i+1, n)]) 
            for i in range(n)
        ])
        
        # Extract off-diagonal elements
        off_diag_labels = labels[i_indices, j_indices]
        off_diag_similarities = similarity_matrix[i_indices, j_indices]
        
        # Reshape to (n, n-1)
        off_diag_labels = off_diag_labels.reshape(n, n-1)
        off_diag_similarities = off_diag_similarities.reshape(n, n-1)
        
        # Separate positives and negatives
        # For each row, find positive and negative similarities
        positives = jnp.where(
            off_diag_labels == 1, 
            off_diag_similarities, 
            -jnp.inf  # Mask out negatives with -inf
        ).max(axis=1, keepdims=True)
        
        # For negatives, we want all the negative similarities
        negatives = jnp.where(
            off_diag_labels == 0,
            off_diag_similarities,
            -jnp.inf  # Mask out positives with -inf
        )
        
        # Combine positives and negatives
        logits = jnp.concatenate([positives, negatives], axis=1)
        
        # Labels for contrastive loss (first position is positive)
        ...
        **kwargs,
        ...
        if contrast_src1 is not None and contrast_src2 is not None:
            ...
            if self.config.get("LINEAR_EMBEDDER"):
                spatial_contrast_embedded1 = (
                    jax.vmap(jax.vmap(self.spatial_embedder))(spatial_contrast1)
                    * self.spatial_scale
                )
            elif self.config.get("EMBED_DIM") == 0:
                spatial_contrast_embedded1 = (
                    jax.vmap(self.spatial_embedder)(spatial_contrast1) * self.spatial_scale
                )
            else:  # config.EMBED_DIM == 1
                spatial_contrast_embedded1 = jax.vmap(jax.vmap(jax.vmap(self.spatial_embedder)))(
                    spatial_contrast1
                )
            ...
            if self.config.get("LINEAR_EMBEDDER"):
                spatial_contrast_embedded2 = (
                    jax.vmap(jax.vmap(self.spatial_embedder))(spatial_contrast2)
                    * self.spatial_scale
                )
            elif self.config.get("EMBED_DIM") == 0:
                spatial_contrast_embedded2 = (
                    jax.vmap(self.spatial_embedder)(spatial_contrast2) * self.spatial_scale
                )
            else:  # config.EMBED_DIM == 1
                spatial_contrast_embedded2 = jax.vmap(jax.vmap(jax.vmap(self.spatial_embedder)))(
                    spatial_contrast2
                )
        ...
        if self.config.get("LOSS").get("TYPE") == "poisson":
            ...
            decoder_loss = poisson_nll_loss(
                decoder_rates, mask_labels, log_input=self.config.get("LOGRATE", False)
            )

        # Extract losses only for masked positions (like PyTorch boolean indexing)
        # valid_mask = mask_labels != UNMASKED_LABEL
        valid_mask = jnp.where(mask_labels != UNMASKED_LABEL, 1.0, 0.0)
        ...
        if not val_phase:
            return (
                loss,  # Primary output: scalar for gradient computation
                (masked_decoder_loss, contrast_loss, decoder_rates),  # Auxiliary output: tuple
            )
        else:
            return (
                loss,
