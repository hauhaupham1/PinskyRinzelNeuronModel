...
def plot_simulation_results(
    ...
    if plot_dendrite and hasattr(sol, "stored_vars"):
        if "Vd" not in sol.stored_vars:
            plot_dendrite = False
            print(
                "Warning: Dendritic voltage (Vd) not stored, dendrite plotting disabled"
            )
    if neurons_to_plot is None:
        neurons_to_plot = range(num_neurons)
    elif not isinstance(neurons_to_plot, Sequence) or not all(
        isinstance(n, int) for n in neurons_to_plot
    ):
    ...
    for sample_idx in range(num_samples):
        ...
        for neuron_index in neurons_to_plot:
            if neuron_index < 0 or neuron_index >= num_neurons:
                print(
                    f"Warning: Neuron index {neuron_index} out of range (0-{num_neurons - 1}). Skipping."
                )
                continue
            ...
            if len(v_data["times"]) > 0:
                ...
                print(
                    f"No valid data found for Neuron {neuron_index} in Sample {sample_idx}."
                )
        ...
        if plot_spikes:
            ...
            if len(plotted_lines) > 0:
                ...
                for spike_t in valid_spike_times:
                    if (
                        spike_t >= min_t_plot and spike_t <= max_t_plot
                    ):  # Ensure spike is within plot range
                        label = "Spike" if not added_spike_legend else ""
                        plt.axvline(
                            spike_t, color="r", linestyle="--", alpha=0.6, label=label
                        )
        ...
        if plotted_lines:
            ...
            if (
                plot_spikes and added_spike_legend
            ):  # Only add spike to legend if it was plotted
                from matplotlib.lines import Line2D

                spike_legend_entry = Line2D(
                    [0], [0], color="r", linestyle="--", alpha=0.6, label="Spike"
                )
