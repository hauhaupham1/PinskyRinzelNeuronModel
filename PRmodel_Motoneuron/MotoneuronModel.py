...
class MotoneuronModel(PinskyRinzel):
    ...
    def analyze_response(self, sol, **kwargs):
        """
        Analyze the neuron's response according to the targets specified in motoneuron.yaml.
        
        Args:
            sol: The solution from solving the ODE
        ...
        # Get spike times
        threshold = kwargs.get('threshold', -37.0)  # From motoneuron.yaml
        spike_times = self.get_spike_times(sol, threshold=threshold)
        results['spike_times'] = spike_times
        
        # Calculate firing rate if there are spikes
        if len(spike_times) > 1:
            # Convert to Hz (spikes per second), noting that times are in ms
            isis = jnp.diff(spike_times)  # Inter-spike intervals
            mean_isi = jnp.mean(isis)
            firing_rate = 1000.0 / mean_isi  # Convert ms to s for Hz
            results['firing_rate'] = firing_rate
        else:
            results['firing_rate'] = 0.0
            
        # Get spike amplitude
        if len(spike_times) > 0:
            # Find indices of spike times
            spike_indices = jnp.searchsorted(times, spike_times)
            # Find local maxima around spike times
            spike_amps = []
            for idx in spike_indices:
                # Look in a window around the spike time
                window_start = max(0, idx - 10)
                window_end = min(len(Vs), idx + 10)
                local_max = jnp.max(Vs[window_start:window_end])
                spike_amps.append(local_max)
            results['spike_amplitudes'] = jnp.array(spike_amps)
            results['mean_spike_amplitude'] = jnp.mean(jnp.array(spike_amps))
        else:
...
def run_motoneuron_example(yaml_file, param_set='20230210_84'):
    """
    Run a simulation using the motoneuron model with parameters from a YAML file.
    ...
    Returns:
        The model, solution, and analysis results
    """
    # Create the model
...
if __name__ == "__main__":
    ...
    # Run the example
    model, sol, results = run_motoneuron_example(yaml_file)
    ...
    if len(results['spike_times']) > 0:
        ...
        print(f"Mean spike amplitude: {results['mean_spike_amplitude']:.2f} mV")
    
    # Plot the results
    import matplotlib.pyplot as plt
    plt.figure(figsize=(12, 6))
    
    # Plot both soma and dendrite on the same graph
    plt.plot(sol.ts, sol.ys[:, 0], label='Vs (Soma)')
    plt.plot(sol.ts, sol.ys[:, 1], label='Vd (Dendrite)')
    plt.xlabel('Time (ms)')
    plt.ylabel('Voltage (mV)')
    plt.title('Motoneuron Model Simulation')
    plt.legend()
    
    plt.tight_layout()
    plt.savefig('motoneuron_simulation.png')
    plt.show()
