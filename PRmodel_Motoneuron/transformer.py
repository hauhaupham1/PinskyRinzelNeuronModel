...
class Head(eqx.Module):
    ...
    def __call__(self, x, key=None):
        ...
        scores = self.dropout(scores, key=key) if key is not None else self.dropout(scores, inference=True)
...
class MultiHeadAttention(eqx.Module):
    ...
    def __call__(self, x, key=None):
        ...
        out = self.dropout(out, key=key) if key is not None else self.dropout(out, inference=True)
        return out
...
class FeedForward(eqx.Module):
    ...
    def __init__(self, n_embed, dropout=0.2, key=None):
        ...
        self.linear1 = eqx.nn.Linear(in_features=n_embed, out_features=4 * n_embed, use_bias=False, key=key1)
        self.activation = eqx.nn.Lambda(jax.nn.relu)
        self.linear2 = eqx.nn.Linear(in_features=4 * n_embed, out_features=n_embed, use_bias=False, key=key2)
        self.dropout = eqx.nn.Dropout(dropout)
    ...
    def __call__(self, x, key=None):
        ...
        x = self.dropout(x, key=key) if key is not None else self.dropout(x, inference=True)
...
class Block(eqx.Module):
    ...
    def __init__(self, n_embed, n_head, max_length=100, dropout=0.2, key=None):
        ...
        self.mha = MultiHeadAttention(n_embed=n_embed, num_heads=n_head, max_length=max_length, dropout=dropout, key=key_mha)
