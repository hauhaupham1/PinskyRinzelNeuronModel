...
def create_mask(data, mode="full", mask_ratio=0.15, key=None):
    if mode not in SUPPORTED_MODES:
        raise ValueError(f"Mode {mode} not supported. Supported modes: {SUPPORTED_MODES}")
...
def apply_masking(data, mask, key, mask_token_ratio=0.8, max_spikes=29, use_zero_mask=True):
    ...
    masked_data = jnp.where(
        ...
        jnp.where(assignment, mask_token_value, random_values),  # Replace with mask token or random
...
def create_reconstruction_mask(batch_data, key, mask_ratio=0.15, mode="full", 
                            mask_token_ratio=0.8, use_zero_mask=True, max_spikes=29):
    ...
    masked_input = apply_masking(batch_data, mask, key2, 
...
def get_mixed_batch(data, forecast_ratio=0.3, num_forward_steps=1, key=None):
    ...
    forecast_input, forecast_labels = create_forward_prediction_mask(forecast_data, num_forward_steps)
