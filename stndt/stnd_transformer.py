...
class STNDT(eqx.Module):
    ...
    def __init__(self, config, trial_length, num_neurons, max_spikes, key=None):
        ...
        if config.get('LINEAR_EMBEDDER'):
            ...
            if config.get('SPIKE_LOG_INIT', False):
                ...
                embed_weights = self.spike_log_embedding_init(num_embeddings, embed_dim, embed_key1)
                spatial_embed_weights = self.spike_log_embedding_init(num_embeddings, embed_dim, embed_key2)
                
                # Create embeddings and replace weights
                embedder_layer = eqx.nn.Embedding(num_embeddings, embed_dim, key=keys[0])
                embedder_layer = eqx.tree_at(lambda m: m.weight, embedder_layer, embed_weights)
                
                spatial_embedder_layer = eqx.nn.Embedding(num_embeddings, embed_dim, key=keys[1])
                spatial_embedder_layer = eqx.tree_at(lambda m: m.weight, spatial_embedder_layer, spatial_embed_weights)
            else:
            ...
            self.embedder = eqx.nn.Sequential([
                embedder_layer,
                eqx.nn.Lambda(lambda x: x.reshape(*x.shape[:-2], -1))  # Flatten last 2 dims 
            ])
        ...
        if config.get('LOSS').get('TYPE') == 'poisson':
            ...
            if config.get('DECODER', {}).get('LAYERS', 1) == 1:
                ...
                weight, bias = self.spike_aware_decoder_init(weight_shape, bias_shape, src_decoder_layers_keys[0])
                ...
                bias_shape = (16,)
                weight, bias = self.spike_aware_decoder_init(weight_shape, bias_shape, src_decoder_layers_keys[0])
                first_layer = eqx.tree_at(lambda m: (m.weight, m.bias), first_layer, (weight, bias))
                
                src_decoder_layers.append(first_layer)
                src_decoder_layers.append(eqx.nn.Lambda(jax.nn.relu))
                src_decoder_layers.append(eqx.nn.Linear(16, self.num_neurons, key=src_decoder_layers_keys[1]))
    ...
    def spike_aware_decoder_init(self, weight_shape, bias_shape, key, initrange=0.1):
        weight = jax.random.uniform(key, weight_shape, minval=-initrange, maxval=initrange)
    ...
    def spike_log_embedding_init(self, num_embeddings, embed_dim, key, initrange=0.1):
        ...
        log_scale = jnp.log(jnp.arange(1, num_embeddings + 1, dtype=jnp.float32))  # [0, ln(2), ln(3), ...]
        log_scale = (log_scale - log_scale.mean()) / (log_scale[-1] - log_scale[0])
        log_scale = log_scale * initrange
        
        # Add some noise
        noise = jax.random.uniform(key, (num_embeddings, embed_dim), 
                                   minval=-initrange/10, maxval=initrange/10)
    ...
    def forward(self, src, mask_labels, contrast_src1=None, contrast_src2=None, val_phase=False, key=None, **kwargs):
        ...
        encoder_output = self.rate_dropout(encoder_output, key=subkey1) if not val_phase else self.rate_dropout(encoder_output, inference=True)
        decoder_output = jax.vmap(jax.vmap(self.src_decoder))(encoder_output)
        ...
        if contrast_src1 is not None and contrast_src2 is not None:
            ...
            if not val_phase:
                contrast_loss = optax.softmax_cross_entropy_with_integer_labels(logits, labels)
                contrast_loss = jnp.mean(contrast_loss) * self.constrast_lambda
            else:
                contrast_loss = optax.softmax_cross_entropy_with_integer_labels(logits, labels) * self.constrast_lambda
            ...
            if self.config.get('CONTRAST_LAYER') == 'embedder':
                ...
                logits, labels = self.info_nce_loss(features)
                if not val_phase:
                    contrast_loss = contrast_loss + jnp.mean(optax.softmax_cross_entropy_with_integer_labels(logits, labels)) * self.constrast_lambda

                else:
                    contrast_loss = contrast_loss + optax.softmax_cross_entropy_with_integer_labels(logits, labels) * self.constrast_lambda
        else:
            contrast_loss = 0.0
        ...
        if self.config.get('LOSS').get('TYPE') == 'poisson':
            ...
            decoder_loss = poisson_nll_loss(decoder_rates, mask_labels, 
                                          log_input=self.config.get('LOGRATE', True))
        ...
        if not val_phase:
            ...
            masked_decoder_loss = valid_losses  # Keep original shape with zeros for invalid
