import os
from get_data_S1 import data_loading_for_batch, load_s1_test, load_s1_train
from built_in_attention import Transformer
import jax
...
def loss_fn(model, inputs, targets, mask_labels, key):
    predictions = model(inputs, enable_dropout=True, key=key)
...
def validate_step(model, inputs, targets, mask_labels, key):
    predictions = model(inputs, enable_dropout=False, key=key)
