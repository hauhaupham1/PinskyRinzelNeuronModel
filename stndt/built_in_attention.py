...
class MultiheadTemporalAttention(eqx.Module):
    ...
    def __call__(self, x, src_mask=None, key=None):
        # x shape: (B, T, N) - need to vmap over batch dimension
        # Built-in attention expects (T, N) per sample
        def single_attention(x_single):
            return self.mha(query=x_single, key_=x_single, value=x_single, mask=src_mask, key=key)
        
        return jax.vmap(single_attention)(x)
...
class STNDT(eqx.Module):
    ...
    def _generate_context_mask(self, size: int, context_forward: int = -1, 
        ...
        """Generate context mask for attention (boolean: True=keep, False=mask)"""
        ...
        # Create forward-looking mask (boolean: True = keep, False = mask out)
        ...
        # Return boolean mask directly (True=keep, False=mask out)
        return mask
